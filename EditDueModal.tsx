...
export default function EditDueModal({ entry, onClose }: EditDueModalProps) {
  const { students, updateDueEntry } = useApp();

  const allFamilyMembers = students.filter((s) =>
    entry.amounts.some((a) => a.studentId === s.id)
  );
  ...
  const [amounts, setAmounts] = useState<Record<string, string>>(() => {
    ...
    entry.amounts.forEach((a) => { init[a.studentId] = String(a.amount); });
    return init;
  });
    ...
    >
      ...
      >
        <div className="sticky top-0 bg-white rounded-t-2xl border-b border-slate-100 px-6 py-4 flex items-center justify-between z-10">
          <h2 className="text-xl font-bold text-slate-900">Edit Due</h2>
